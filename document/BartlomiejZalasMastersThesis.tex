...
\subsubsection{Automated Performance Tuning System (APTS)}
...
The \textit{APTS Manager} is main part of the system. It is responsible for enabling performance tuning by performing performance evaluation tests and reporting performance issues raised by tests to \textit{Decision Module}. 
...
The \textit{Decision Module} is responsible for performance issue analysis and performance tuning. This feature is realized on form of rule based system in which performance issues are conditions and performance tuning decisions are actions.   
...
The \textit{Application Server} node is designate only to host tested application and is independent from the rest of the system - in case of \textit{Configuration Server} unavailability predefined default configuration is used. When \textit{InfluxDB} is offline no performance measurement are stored. 
...
\begin{itemize}
\item Plain Old Java Object based
...
\textbf{@PetCase} & Annotation used to mark method to be interpreted as PET case. Available parameters are presented in the Table \ref{petcase}.
\end{tabularx}
\end{table}
...
\begin{itemize} 
\item \textit{kpiName} - identifier of KPI used in performance issue. 
...
void assertKpiLessThan(String kpiName, Double currentValue, Double limit);

void assertKpiGreaterThan(String kpiName, Double currentValue, Double limit);

void assertKpiEqual(String kpiName, Double currentValue, Double expectedValue);
\end{minted}
...
\item \textit{kpiName} - information about KPI which was different than expected
...
In order to run tests cases the PET Framework provides runner (PetCaseRunner) which will run all tests cases marked with proper annotations from the give package. Because the need of continuous monitoring of the reported performance issues, the runner is implemented as thread and provides asynchronous API to retrieve new reported issues. The Listing \ref{petrunner} presents sample usage of the PetCaseRunner. 
