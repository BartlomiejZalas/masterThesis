...
As presented on Figure \ref{componentapts}, the system is composed of the following components: \textit{Workload Generator}, \textit{APTS Manager} which includes \textit{PET Cases} and \textit{Decision Module}, \textit{InfluxDB com.zalas.masterthesis.resourcemonitoringservice.service.Service}, \textit{Configuration com.zalas.masterthesis.resourcemonitoringservice.service.Service} and \textit{Tested Application}.
...
The \textit{PET Cases} component contains performance evaluation scenarios written with help of PET Framework (see Chapter \ref{framework}). The tests monitor the tested application in specified schedule, gather monitored KPIs from \textit{InfluxDB com.zalas.masterthesis.resourcemonitoringservice.service.Service} and asserts KPIs values. In case of failed assertion performance issue is raised.
...
The \textit{InfluxDB com.zalas.masterthesis.resourcemonitoringservice.service.Service} hosts InfluxDB \cite{influxdb} time series database which provides HTTP API for querying and writing data. The database stores performance measurements which are generated by the tested applications and required by the performance evaluation tests.

The \textit{Configuration com.zalas.masterthesis.resourcemonitoringservice.service.Service} is external service where performance tuning configuration is stored (active redundant component and current application server thread number). The \textit{Configuration com.zalas.masterthesis.resourcemonitoringservice.service.Service} provides REST API which enables different parts of the system to check and change current configuration of the tested application.
...
The system, as presented on Figure \ref{deploymentapts}, is deployed on \textit{Client Workstation}, \textit{Application Server} and \textit{com.zalas.masterthesis.resourcemonitoringservice.service.Service Server}.
...
The \textit{com.zalas.masterthesis.resourcemonitoringservice.service.Service Server} node holds additional services used by the APTS. This node was separated from the \textit{Application Server} node in order to minimize overhead of the APTS on the tested application.
