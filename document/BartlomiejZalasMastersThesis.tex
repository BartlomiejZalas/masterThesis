...
As mentioned in previous chapters performance optimization can be developed in many areas. For this work author will focus on web based application. The application will be a service simulating product database - basic CRUD operations on products categories will be provided. For simplicity and in order to concentrate on the topic of this work graphical user interface will be omitted. Communication with application will be provided in form of REST service.
...
\noindent\textbf{Execute task} - starts task which will simulate high CPU usage (40th number of Fibonacci sequence). \\
\\
  \begin{tabular}{ll}
  Method: & GET \\
  Path: & /task/ \\
...
In order to make right choices about performance improvement realization set of experiments must be performed. 
For that purpose 10000 product categories with related entities where generated. Each product category may have from 1 to 4 products. Each product may have from 1 to 4 opinions. Exact number of related entities was chosen randomly during generation. In summary number of created entities for experiments is as follows:
\begin{itemize}
\item Product category: 10000 entities
\item Product: about 25000 entities
\item Product opinion: about 62500 entities
\end{itemize}  

Execution time of methods was measured by aspect oriented class. The class wrapped method by two dates: start date (measured before method execution) and end date (measured after method execution). Difference between the start and the end date is equal to the execution of method. The execution time was logged and persisted in a time series database (InfluxDB) in order to enable further analysis. 

Required workload was simulated by tests scenarios generated in Gatling Load Testing engine. Following test scenarios was developed:
\begin{itemize}
...
\item \textbf{Insert} - generates 20 parallel requests to "Add new product category" method. 
\end{itemize}

In the first experiment different paging mechanism was compared. The execution time for each mechanism was measured by summing up execution time needed to retrieve all 10000 product categories from the tested application.   
...
\begin{tikzpicture}
        ...
        \end{axis}
\end{tikzpicture}
\caption{\textit{Execution time comparision for different page size for 10000 product categories}} \label{fig:pagesizetime}
\end{figure}
...
Number of threads has key meaning in handling incoming requests into an application server. When number of threads is less than parallel incoming requests not handled part will be waiting in the queue until threads will be free again. Too large number of threads results in increased CPU usage and slower performance. This fact is documented in the Figure \ref{fig:threads}. For 100 incoming parallel requests tested 4 configuration of thread number: 2 threads, 10 threads, 25 threads, 100 threads. As one can see the worst mean response time was when to many parallel threads was executed or when number of threads was not sufficiently large to handle incoming requests.    

\pgfplotstableread[row sep=\\,col sep=&]{
    category           & t2    & t10    & t25   & t100\\
    Min response time  & 2091  & 3926   & 10706 & 32067\\
    Max response time  & 56558 & 38739  & 38327 & 35279\\
    Mean response time & 29641 & 21531  & 25213 & 34216\\
    }\mydata
\begin{figure}[!htb]
\centering
\begin{tikzpicture}
    \begin{axis}[
        ...
        \addplot[style={bblue,fill=bblue,mark=none}] table[x=category,y=t2]{\mydata};
        \addplot[style={rred,fill=rred,mark=none}] table[x=category,y=t10]{\mydata};
        \addplot[style={ggreen,fill=ggreen,mark=none}] table[x=category,y=t25]{\mydata};
        \addplot[style={ppurple,fill=ppurple,mark=none}] table[x=category,y=t100]{\mydata};
        \legend{2 threads, 10 threads, 25 threads, 100 threads}
    \end{axis}
\end{tikzpicture}
