...
\noindent\textbf{Execute wait task} - starts task which will simulate waiting task without CPU consumption. Real life example of such task could process which sends request to an external service (for example by HTTP) and waits for a response. The task "wait" time is defined by the parameter \textit{sleepTime}. 
...
\begin{listing}[ht]\begin{minted}{java}
/*
 * imports
 */
@Pet
...
All the experiment was performed on the following machine:
...
The initial configuration provided by the \textit{Configuration Service} during the experiments evaluating the APTS presented in the Table \ref{cacheinitconf}. As presented, by default cache and batch are disabled, the application server threads number is set to 20. 

\begin{table}[!htb]
\caption{\textit{Initial configuration of the tested application during the experiments}} \label{cacheinitconf}
\begin{tabularx}{\textwidth}{X|X}
...
\subsubsection{The Results} \label{section:batchresults}
...
\begin{tabular}{c| C{1cm} C{1.3cm} C{1.5cm}|C{1.3cm} C{1.5cm} | C{1.3cm} C{1.5cm} |C{1cm}}
    ...
    10 & 30 & 38.6\% & 5.0 & 100.0\% & 22.4 & 66.1\% & 9.4 \\ \cline{1-9}
   	   & 32 & 41.8\% & 5.0 & 99.9\% & 20.1 & 68.7\% & 8.8 & \multicolumn{1}{c}{Average} \\ 
\end{tabular}
\end{center}
\label{appendixmeasurementscache}
\end{table}
